  ...
  parseMtlFile(mtlFile) {
    ...
    for (let currLine = 0; currLine < mtlLines.length; currLine++) {
      ...
      if (line.startsWith("newmtl")) {
        ...
        this.diffuseMap.set(currMaterial, [
          parseFloat(values[0]),
          parseFloat(values[1]),
          parseFloat(values[2]),
          1.0,
        ...
        let values = line.match(/[+-]?([0-9]+[.])?[0-9]+/g);
        this.specularMap.set(currMaterial, [
          parseFloat(values[0]),
          parseFloat(values[1]),
          parseFloat(values[2]),
          1.0,
  ...
  parseObjFile(objFile) {
    ...
    for (let currLine = 0; currLine < objLines.length; currLine++) {
      ...
      if (line.startsWith("vn")) {
        ...
        // Vertex UV definition (texture coordinate)
        let coords = line.match(/[+-]?([0-9]+[.])?[0-9]+/g);
        uvs.push(vec2(coords[0], 1.0 - coords[1]));
      } else if (line.charAt(0) === "v") {
        // Vertex position definition
        let coords = line.split(" ");
        vertices.push(vec4(coords[1], coords[2], coords[3], 1.0));
    ...
    if (types === 0) {
      ...
      indices.forEach((value) => {
        ...
        if (secondSlashIndex > firstSlashIndex + 1) {
          faceTexs.push(
            this.getVertex(value, firstSlashIndex + 1, secondSlashIndex, uvs),
          );
        }

        // Normal coordinates
        faceNorms.push(
          this.getVertex(value, secondSlashIndex + 1, -1, normals),
        );
      });
    }
